		...
		if($this->session->has_userdata('id_usuario'))
   		{     				
			...
			if(($data['session']['tipo'] === 'ADMINISTRADOR'))
			{
				...
				array(
	              	'Pagina Principal'  	=> 'panel/index',
	              	'Gestor de Colores'	=> $this->controller.'/index',
	              	'Registrar Color'		=> '',              	
	            );
				...
				if($check)
		      	{
		           	$data['alert']['success'] = 
						array( 
							'Colores Eliminado Exitosamente',				
						);
		      	}
		      	else
		      	{         
		           	$data['alert']['danger'] = 
						array( 
							'No Existe Colores ó No es posible eliminar por motivos de seguridad',				
						);
		      	}
