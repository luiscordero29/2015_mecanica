		...
		if($this->session->has_userdata('id_usuario'))
   		{     				
			...
			if(($data['session']['tipo'] === 'ADMINISTRADOR'))
			{
				...
				array(
	              	'Pagina Principal'  	=> 'panel/index',
	              	'Gestor de Administradores'	=> $this->controller.'/index',
		...
		if($this->session->has_userdata('id_usuario'))
   		{     				
			...
			if(($data['session']['tipo'] === 'ADMINISTRADOR'))
			{
				...
				$this->form_validation->set_rules('usuario', 'Usuario', 'trim|required|callback_usuario_check|min_length[6]|max_length[15]|alpha_numeric');
