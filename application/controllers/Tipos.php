		...
		if($this->session->has_userdata('id_usuario'))
   		{     				
			...
			if(($data['session']['tipo'] === 'ADMINISTRADOR'))
			{
				...
				array(
	              	'Pagina Principal'  	=> 'panel/index',
	              	'Gestor de Tipos'	=> $this->controller.'/index',
	              	'Registrar Tipo de Veh&iacute;culo'		=> '',              	
	            );
	            
	            $data['controller'] = $this->controller;
				
				$this->form_validation->set_rules('tipo', 'Tipo de Veh&iacute;culo', 'trim|required');
			...
			if(($data['session']['tipo'] === 'ADMINISTRADOR'))
			{
				...
				if($check)
		      	...
		      	{         
		           	$data['alert']['danger'] = 
						array( 
							'No Existe Tipos ó No es posible eliminar por motivos de seguridad',				
						);
		      	}
